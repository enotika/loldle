...
class ClassicGame(tk.Frame):
    def __init__(self, parent):
        ...
        label = tk.Label(self, text="Это игра Классика!")
        label.pack(pady=50)
        ...
        # Подписи для полей
        self.labels = ["Чемпион", "Пол", "Позиция(и)", "Виды", "Ресурс", "Тип диапазона", "Регион(ы)", "Год выпуска"]
    ...
    def add_row(self, guessed_properties, partly_guessed_properties, properties):
        ...
        label = tk.Label(row_frame, text=properties["answer"],
                         image=photo,
        ...
        for key, value in guessed_data.items():
            ...
            if key == "Год выпуска":
                txt = ", ".join(value)
                if int(value[0]) > int(first_terminal.selected_answer["properties"]["Год выпуска"][0]):
                    txt += "⬇️"
                elif int(value[0]) < int(first_terminal.selected_answer["properties"]["Год выпуска"][0]):
                    txt+= "⬆️"
                label = tk.Label(row_frame, text=txt,
                                borderwidth=2,  # Ширина обводки
                                relief="solid",  # Стиль обводки
                ...
                label = tk.Label(row_frame, text=", ".join(value),
                                borderwidth=2,  # Ширина обводки
                                relief="solid",  # Стиль обводки
                                background=color,  # Цвет заливки в зависимости от статуса
                                padx=5,  # Внутренний отступ по горизонтали
    ...
    def get_text_from_button(self):
        ...
        guessed_properties, partly_guessed_properties, guessed_data = first_terminal.check(user_input)

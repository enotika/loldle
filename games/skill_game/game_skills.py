...
class SkillGame(tk.Frame):
    def __init__(self, parent):
        ...
        # Заголовок
        self.title_label = tk.Label(self, text="Игра skills", font=("Arial", 24))
        ...
        # Поле для ввода
        self.input_label = tk.Label(self, text="Введите название картинки:")
        self.input_label.pack(pady=5)

        self.input_field = tk.Entry(self, font=("Arial", 14))
        ...
        # Кнопка проверки
        self.check_button = tk.Button(self, text="Проверить", command=self.check_answer, font=("Arial", 14))
    ...
    def create_buttons(self):
        ...
        self.button_p = tk.Button(self.buttons_frame, text=button_texts[0], command=lambda: self.button_action(button_texts[0]), font=("Arial", 14))
        self.button_p.pack(side=tk.LEFT, padx=5)
        # Создаем первую кнопку
        self.button_q = tk.Button(self.buttons_frame, text=button_texts[1], command=lambda: self.button_action(button_texts[1]), font=("Arial", 14))
        self.button_q.pack(side=tk.LEFT, padx=5)
        # Создаем первую кнопку
        self.button_w = tk.Button(self.buttons_frame, text=button_texts[2], command=lambda: self.button_action(button_texts[2]), font=("Arial", 14))
        self.button_w.pack(side=tk.LEFT, padx=5)
        # Создаем первую кнопку
        self.button_e = tk.Button(self.buttons_frame, text=button_texts[3], command=lambda: self.button_action(button_texts[3]), font=("Arial", 14))
        self.button_e.pack(side=tk.LEFT, padx=5)
        # Создаем первую кнопку
        self.button_r = tk.Button(self.buttons_frame, text=button_texts[4], command=lambda: self.button_action(button_texts[4]), font=("Arial", 14))
    ...
    def load_random_image(self):
        ...
        image_folder_addres = "data/champ_images/" + self.random_folder + "/" + self.random_folder + "_skills"
    ...
    def check_answer(self, event=None):
        ...
        if answer in [i.strip().lower().replace(" ", "") for i in self.all_answers]:  # Преобразуем все ответы в нижний регистр для сравнения
            self.all_answers = [i for i in self.all_answers if i.strip().lower().replace(" ", "") != answer]  # Удаляем значение без учета регистра
        else:
            return
        ...
        if answer == self.random_folder.lower():
            ...
            max_size = 300
            self.image.thumbnail((max_size, max_size))  # Пропорциональное уменьшение изображения
            self.image_tk = ImageTk.PhotoImage(self.image)
